defmodule Protobuf.Protoc.Generator.Message do
  ...
  defp gen_fields(syntax, fields) do
    Enum.map(fields, fn f -> gen_field(syntax, f) end)
  end

  defp gen_field(:proto3, f = %{proto3_optional: true, label: "optional", opts_str: opts_str}) do
    ":#{f[:name]}, #{f[:number]}, proto3_optional: true, type: #{f[:type]}#{opts_str}"
  end

  defp gen_field(syntax, f = %{opts_str: opts_str}) do
